...
/**
 * Helper to find all Firestore doc snapshots for a volume and issue, regardless of string/number type or doc ID format.
 */
    ...
    try {
      ...
      if (matchedDocs.length > 0) {
        for (const docSnapshot of matchedDocs) {
          await setDoc(doc(db, 'issues', docSnapshot.id), { isPublished: Boolean(isPublished) }, { merge: true });
        }
      } else {
        const defaultDocId = `vol-${volume}-iss-${issue}`;
        await setDoc(doc(db, 'issues', defaultDocId), { isPublished: Boolean(isPublished) }, { merge: true });
      }
    } catch (err) {
